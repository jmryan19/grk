...
def main(args: dict):
    if args.wandb_offline:
        ...
        mode = 'online'
    wandb.init(entity='jmryan', project="emergence_redo",name="pure_openai", mode=mode, config=args)
    # TODO: add wandb name
    ...
    wandb.define_metric("training/avg_predicted", step_metric='step')
...
def train(model, train_loader, optimizer, scheduler, device, num_steps, num_classes, loss_arg):
    ...
    for batch in train_loader:
        ...
        metrics = {
            ...
            "training/loss": loss,
            "training/avg_predicted": torch.argmax(output, dim=1).float().mean(),
            "step": wandb.run.step
        }
        wandb.log(metrics)
